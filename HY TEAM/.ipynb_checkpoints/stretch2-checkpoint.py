...
if __name__ == "__main__":
    ...
    match_mode = 'scene'  # 匹配模式，可选'semantic'或'scene'
    ...
    # 直接使用给定的四个点进行视觉对齐
    src_points = [(106, 70), (1647, 78), (1698, 870), (65, 814)]  # 红外视频点
    dst_points = [(10, 94), (592, 109), (633, 431), (8, 369)]  # 热成像视频点
    ...
    fusion_output_path = os.path.join(output_dir, 'fusion_result.mp4')  # 新增融合结果视频路径
    ...
    fusion_writer = cv2.VideoWriter(fusion_output_path, fourcc, output_fps,
                                    (infrared_width, infrared_height))  # 新增融合结果视频写入器
    ...
    while infrared_frame_idx < infrared_frame_count:
        ...
        # 透视变换后，拉伸热成像视频
        thermal_frame_resized = cv2.warpPerspective(thermal_frame, M, (infrared_width, infrared_height))
        ...
        # 引导滤波
        gray_dehazed_infrared_frame = cv2.ximgproc.guidedFilter(guide=gray_infrared_frame,
                                                                src=gray_dehazed_infrared_frame,
        ...
        if elapsed_since_last_print >= 1.0:  # 每秒打印一次
            ...
            print(
                f"处理进度: {total_frames_processed}/{infrared_frame_count} 帧 ({progress_percent:.1f}%) - 当前帧率: {fps:.2f} fps - 平均帧率: {avg_fps:.2f} fps")
